...
public class BackoffRetryCounter {
    ...
    public synchronized void reset() {
        if (intervalIndex > 0) {
            log.debug("Backoff retry counter reset to initial interval");
        }
        intervalIndex = 0;
    }
    ...
    public synchronized void increment() {
        if (intervalIndex < timeIntervals.length - 1) {
            intervalIndex++;
            log.debug("Backoff retry counter incremented. Next interval: {}", timeIntervalNames[intervalIndex]);
        } else {
            log.warn("Backoff retry counter at maximum interval: {}", timeIntervalNames[intervalIndex]);
        }
    }
